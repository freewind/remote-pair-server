...
import in.freewind.intellij.remotepair.{BuildInfo, MySpecification}
import in.freewind.intellij.remotepair.server.{Project, Projects}
...
class CreateOrJoinProjectSpec extends MySpecification {
  ...
  "User who has not joined to any project" should {
    ...
    "not send WatchFilesRequest related request" in new ProtocolMocking {
      cannotSendEvents(WatchFilesRequest(Seq("/aaa")))
    }
