...
class AdjustedStringOpsSpec extends Specification {
  ...
  "如果两人几乎同时对同一份文档做了一个操作,后者的操作可能需要调整,才能在前者之后应用到文档上" should {
    ...
    "对于两者都是添加操作,第二个人的操作可能会被调整" should {
      ...
      "如果第二个人添加的内容是第一个人的一部分,且从同一个位置添加,则第二个人的操作会被调整为空" in {
        StringDiff.adjustLaterOps(Seq(Insert(10, "abc")), Seq(Insert(10, "ab"))) should beEmpty
      }
      "如果第二个人添加的内容比第一个人多,且从同一个位置添加,则相同部分会被省略" in {
        StringDiff.adjustLaterOps(Seq(Insert(10, "abc")), Seq(Insert(10, "abcde"))) === Seq(Insert(13, "de"))
      }
      "如果第二个人添加内容不属于前几种情况,且起点在第一个人的起点位置或后面,则起点需要调整" in {
        StringDiff.adjustLaterOps(Seq(Insert(10, "abc")), Seq(Insert(10, "bc"))) === Seq(Insert(13, "bc"))
      }
      "如果第二个人添加内容不属于前几种情况,且起点在第一个人的起点前面,则完全不需要调整" in {
        StringDiff.adjustLaterOps(Seq(Insert(10, "abc")), Seq(Insert(9, "bc"))) === Seq(Insert(9, "bc"))
      }
    }
    ...
    "如果第一个人是添加操作,第二个人是删除操作,则后者的内容不需要调整,但起点有可能调整" should {
      ...
      "如果后者的起点小于前者的起点,则起点不需要调整,但要避开前者添加的内容" in {
        StringDiff.adjustLaterOps(Seq(Insert(10, "bc")), Seq(Delete(9, 5))) === Seq(Delete(9, 1), Delete(12, 4))
      }
    }
  }
  ...
  "如果两人几乎同时对同一份文档做了多个操作,后者的操作可能需要调整,才能在前者之后应用到文档上" in {
    ...
    StringDiff.adjustLaterOps(
      Seq(Insert(8, "xy"), Delete(11, 5), Insert(20, "op")),
      Seq(Insert(3, "abc"), Delete(10, 8), Delete(16, 4), Insert(30, "mn"))
    ) === Seq(Insert(3, "abc"), Delete(11, 4), Delete(17, 3), Delete(22, 1), Insert(33, "mn"))
  }
}
